...
static char buffer[SHAREDMEM_SIZE];
...
main(int argc, char *const *argv)
{
 ...
 //------------------------------set sockfd.ini(0:don't close;1:to close)--------------------------   
    if((fpsock = fopen("/usr/local/nginx-1.4.2-mem/sockfd.ini","r")) == NULL)
    {
    	if((fpsock = fopen("/usr/local/nginx-1.4.2-mem/sockfd.ini","w+")) == NULL)
    	{
...
//----------------------------set forkonce--------------------------------------------------------
    if((fpfork = fopen("/usr/local/nginx-1.4.2-mem/forkonce.ini","r+")) == NULL)
    {
    	if((fpfork = fopen("/usr/local/nginx-1.4.2-mem/forkonce.ini","w+")) == NULL)
    	{
...
//*********************************************************************1
            		...
            		else
            		{
            			...
            			{
            				fclose(fpfork);	
//-----------------------save the process id of myServer ------------------------------------//
					fflush(stdout);
            				fpid = fork();

            				if( ((int)fpid) != 0 ) 
            				{
            					FILE* out = fopen("/usr/local/nginx-1.4.2-mem/pid","w+");
...
//---------------------------------------------------------------------------------------------//
            				...
            				{
            					printf("invoke myserver()\n");	// print whether myserver process is invoked				
            					myServer();

						printf("myserver is killed!\n");	
            					return 0;
            				}
            				if((fpfork = fopen("/usr/local/nginx-1.4.2-mem/forkonce.ini","w")) == NULL)
            				{
...
//---------------------------------some operation to release resource---------------------------------//
                        FILE* log = fopen("/usr/local/nginx-1.4.2-mem/logs/my_log.log","w");
                        fclose(log);

                        //get process id of myServer
                  			int Pid;
                  			FILE* in = fopen("/usr/local/nginx-1.4.2-mem/pid","r");
                        ...
                        //delete semaphore
                        int semID = semget(ftok("/usr/local/nginx-1.4.2-mem/applicationList.xml",'a'),0,IPC_CREAT|0666);
