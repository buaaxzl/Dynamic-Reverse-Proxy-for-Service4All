...
//*******************************************1
#define NGX_CONF_PATH	"/usr/local/nginx-1.4.2-mem/conf/nginx.conf"
//*******************************************2
...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
	...
	if(strcmp(filename->data, NGX_CONF_PATH) == 0)
	{
		fd = 4;
		cf->nginxConf_fd = fd;
	}else
...
//*******************************************1
	...
	if(cf->conf_file->file.fd == cf->nginxConf_fd)	
	{   
	    file_size = cf->conf_mem->mem->len;         
	}
	else {  
...
//*******************************************1
	    cf->conf_mem->mem->offset = cf->conf_file->file.offset;	//important
//*******************************************2
...
//*******************************************1
	    if(cf->conf_file->file.fd == cf->nginxConf_fd) 
	    {
            //printf("-----------------conf_read_token---2\n");
	    	n = ngx_read_conf_Memory(cf->conf_mem->mem, &cf->conf_file->file, b->start + len, size, cf->conf_mem->mem->offset);          
        }
	    else
        {
            //printf("-----------------conf_read_token---3\n");
