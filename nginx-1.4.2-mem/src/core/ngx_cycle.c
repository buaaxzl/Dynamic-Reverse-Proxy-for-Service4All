...
//------------------
//*******************************************1
#define SHAREDMEM_SIZE_1 4096*90
#define SHAREDMEM_NAME 1236
static int tempConf_count = 0;
//*******************************************2

//---------------------------------------
static char* nginxPort = "5800";
static char* nginxIp = "127.0.0.1";
//---------------------------------------
...
//*******************************************1	
//------------------------Shared Memory    Read ----------
	int shmid_2 = shmget((key_t)SHAREDMEM_NAME,SHAREDMEM_SIZE_1,IPC_CREAT|0666);
	if(shmid_2 == -1)
	{	
	...
	char* shared_memory_2 = shmat(shmid_2,(void *)0, 0);
	...
	char text[SHAREDMEM_SIZE_1];
	strcpy(text, shared_memory_2);
...
//------------------------------------------------------------	
	...
	if(tempConf_count == 0)
	{
//-----------------------read information from applicationList.xml --------------------------		
        tempConf_count++;
		xmlInitParser();
        xmlKeepBlanksDefault(0);
		printf("tempConf_count = %d\n",tempConf_count);
		...
		strcat(upstream,tempConf);
				
		xmlNodePtr curNode = NULL;
		xmlDocPtr infoListDoc = NULL;
		xmlNodePtr root = NULL;

        if(fopen("/usr/local/nginx-1.4.2-mem/applicationList.xml","r") == NULL )
        {
            FILE* in = fopen("/usr/local/nginx-1.4.2-mem/applicationList.xml","w");
            fprintf(in,"%s","<?xml version=\"1.0\" encoding=\"UTF-8\"?><deployedApplication></deployedApplication>");
            fclose(in);
        }
...
//---------------------------------------generate conf file----------------------------				
        ...
        //strcat(location,location_added);
       	strcat(location,php_location);  //add php_support to nginx
		...
		memString.data = tempConf;
		memString.len = strlen(tempConf);	
//--------------------------------------------------------------------------------------	
	}
	else
	{
		memString.data = text;
		memString.len = strlen(text);
	}
	
	memString.offset = 0;
	ngx_conf_mem_t confmem;
	confmem.mem = &memString;
	conf.conf_mem = &confmem;
//*******************************************2
